...
void load_op(volatile int op[], hls::stream<int>&ALU_operation)
{
 ...
 l_operation: for(int i = 0; i < 50; i++)
 {
#pragma HLS PIPELINE II=1
 ...
 switch(ALU_operation_MEM[i])
  {
   ...
   case 0 :
    data_result.write(data_a.read() + 27);
    data_b.read();
   break;

   case 1 :
    data_result.write(data_b.read() + 27);
    data_a.read();
    break;
...
{
 int i = 0;
 write_back: while(!data_result.empty())
 {
#pragma HLS PIPELINE II=1
...
#pragma HLS STREAM variable=data_a depth=50


 static hls::stream<int> data_b("data_b");
#pragma HLS STREAM variable=data_b depth=50


 static hls::stream<int> data_result("data_result");
#pragma HLS STREAM variable=data_result depth=50
