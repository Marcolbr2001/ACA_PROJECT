...
entity alv_VHDL_control_s_axi is
generic (
...
end entity alv_VHDL_control_s_axi;
...
architecture behave of alv_VHDL_control_s_axi is
    ...
    constant ADDR_SELEC_DATA_0 : INTEGER := 16#40#;
    constant ADDR_SELEC_CTRL   : INTEGER := 16#44#;
    ...
    signal int_selec           : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SELEC_DATA_0 =>
                        rdata_data <= RESIZE(int_selec(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    selec                <= STD_LOGIC_VECTOR(int_selec);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SELEC_DATA_0) then
                    int_selec(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_selec(31 downto 0));
                end if;
            end if;
        end if;
    end process;
