...
__attribute__((sdx_kernel("dec_MIMD", 0))) void dec_MIMD(volatile int* a, volatile int* b, volatile int* c, volatile int* op, volatile int group) {
#line 15 "C:/Users/lotto/Desktop/Alveare/M_AXI_ALU/M_AXI_ALU/dec_MIMD/dec_MIMD/solution1/csynth.tcl"
...
#pragma HLS INTERFACE mode=m_axi port=op bundle=gmem3 depth=50 offset=slave
 ...
 if (group<50/10)
 {
  ...
  VITIS_LOOP_45_2: for(int i=0; i < 10; i++)
  {
 ...
 {
  VITIS_LOOP_99_3: for(int i = 0; i < 10; i++)
   {
    data_result[i] = 0;
   }
 }
 ...
 VITIS_LOOP_106_4: for(int i = 0; i < 10; i++)
 {
  c[i] = data_result[i];
 }
