...
//------------------------Parameter----------------------
`define TV_IN_a "../tv/cdatafile/c.dec_MIMD.autotvin_a.dat"
`define TV_IN_b "../tv/cdatafile/c.dec_MIMD.autotvin_b.dat"
`define TV_IN_c "../tv/cdatafile/c.dec_MIMD.autotvin_c.dat"
`define TV_IN_op "../tv/cdatafile/c.dec_MIMD.autotvin_op.dat"
`define TV_IN_group_r "../tv/cdatafile/c.dec_MIMD.autotvin_group_r.dat"
...
parameter op_c_bitwidth = 64;
parameter group_r_DEPTH = 1;
reg [31 : 0] group_r_OPERATE_DEPTH = 0;
parameter group_r_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter dec_MIMD_continue_addr = 0;
parameter dec_MIMD_auto_start_addr = 0;
parameter a_data_in_addr = 16;
parameter b_data_in_addr = 28;
parameter c_data_in_addr = 40;
parameter op_data_in_addr = 52;
parameter group_r_data_in_addr = 64;
...
reg [op_c_bitwidth - 1 : 0] mem_op [op_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_op [ (op_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * op_DEPTH -1 : 0] = '{default : 'hz};
reg op_write_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_group_r [group_r_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_group_r [ (group_r_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * group_r_DEPTH -1 : 0] = '{default : 'hz};
reg group_r_write_data_finish;
...
reg [31 : 0] op_diff_count = 0;
reg write_op_run_flag = 0;
reg write_one_op_data_done = 0;
//write group_r reg
reg [31 : 0] write_group_r_count = 0;
reg [31 : 0] group_r_diff_count = 0;
reg write_group_r_run_flag = 0;
reg write_one_group_r_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & a_write_data_finish & b_write_data_finish & c_write_data_finish & op_write_data_finish & group_r_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        else if ((write_one_op_data_done == 1 && write_op_count == op_diff_count - 1) || op_diff_count == 0) begin
            write_op_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_op_run_flag == 1 && write_op_count == op_diff_count) begin
            op_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_op
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    integer c_bitwidth;
    integer i;
    ...
    while (1) begin
        ...
        for (k = 0; k < op_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (op_c_bitwidth < 32) begin
                    op_data_tmp_reg = mem_op[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < op_c_bitwidth) begin
                            op_data_tmp_reg[j] = mem_op[k][i*32 + j];
                        end
                        else begin
                            op_data_tmp_reg[j] = 0;
                        end
                ...
                if(image_mem_op[k * four_byte_num  + i]!==op_data_tmp_reg) begin
                op_diff_count = op_diff_count + 1;
                end
            end
        end
...
initial begin : write_op
    ...
    while (1) begin
        ...
        for (check_op_count = 0; check_op_count < op_OPERATE_DEPTH; check_op_count = check_op_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_op_run_flag === 1 && get_ack === 1) begin
                    ...
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (op_c_bitwidth < 32) begin
                            op_data_tmp_reg = mem_op[check_op_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < op_c_bitwidth) begin
                                    op_data_tmp_reg[j] = mem_op[check_op_count][i*32 + j];
                                end
                                else begin
                                    op_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_op[check_op_count * four_byte_num  + i]!==op_data_tmp_reg) begin
                        write (op_data_in_addr + check_op_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, op_data_tmp_reg, write_op_resp);
                        write_one_op_data_done <= 1;
                        @(posedge clk);
                        write_one_op_data_done <= 0;
                        image_mem_op[check_op_count * four_byte_num + i]=op_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_group_r_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (group_r_c_bitwidth, group_r_DEPTH, group_r_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_group_r_run_flag <= 1; 
        end
        else if ((write_one_group_r_data_done == 1 && write_group_r_count == group_r_diff_count - 1) || group_r_diff_count == 0) begin
            write_group_r_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_group_r_count = 0;
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_group_r_count = 0;
        end
        if (write_one_group_r_data_done === 1) begin
            write_group_r_count = write_group_r_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        group_r_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            group_r_write_data_finish <= 0;
        end
        if (write_group_r_run_flag == 1 && write_group_r_count == group_r_diff_count) begin
            group_r_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_group_r
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    integer k;
    reg [31 : 0] group_r_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = group_r_c_bitwidth;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num);
    ceil_align_to_pow_of_two_four_byte_num = ceil_align_to_pow_of_two(four_byte_num);
    while (1) begin
        wait (AESL_ready_reg === 1);
        group_r_diff_count = 0;

        for (k = 0; k < group_r_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (group_r_c_bitwidth < 32) begin
                    group_r_data_tmp_reg = mem_group_r[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < group_r_c_bitwidth) begin
                            group_r_data_tmp_reg[j] = mem_group_r[k][i*32 + j];
                        end
                        else begin
                            group_r_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_group_r[k * four_byte_num  + i]!==group_r_data_tmp_reg) begin
                group_r_diff_count = group_r_diff_count + 1;
                end
            end
        end
...
initial begin : write_group_r
    integer write_group_r_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_group_r_count = 0; check_group_r_count < group_r_OPERATE_DEPTH; check_group_r_count = check_group_r_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_group_r_run_flag === 1 && get_ack === 1) begin
                    process_busy = 1;
                    //write group_r data 
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (group_r_c_bitwidth < 32) begin
                            group_r_data_tmp_reg = mem_group_r[check_group_r_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < group_r_c_bitwidth) begin
                                    group_r_data_tmp_reg[j] = mem_group_r[check_group_r_count][i*32 + j];
                                end
                                else begin
                                    group_r_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_group_r[check_group_r_count * four_byte_num  + i]!==group_r_data_tmp_reg) begin
                        write (group_r_data_in_addr + check_group_r_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, group_r_data_tmp_reg, write_group_r_resp);
                        write_one_group_r_data_done <= 1;
                        @(posedge clk);
                        write_one_group_r_data_done <= 0;
                        image_mem_group_r[check_group_r_count * four_byte_num + i]=group_r_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
// Read data from file 
initial begin : read_group_r_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [group_r_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (group_r_c_bitwidth , factor);
  fp = $fopen(`TV_IN_group_r ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_group_r); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < group_r_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_group_r [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
